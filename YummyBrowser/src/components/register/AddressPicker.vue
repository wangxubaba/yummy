<template>
    <div>
      <Select class="select" @on-change="changeProvince" placeholder="省" v-model="province">
        <Option v-for="(province, index1) in addresses" :value="index1" :key="province.name">{{province.name}}</Option>
      </Select>
      <Select class="select" @on-change="changeCity" :disabled="validCity" placeholder="市" v-model="city">
        <Option v-for="(city, index2) in cities" :value="index2" :key="city.name">{{city.name}}</Option>
      </Select>
      <Select class="select" @on-change="changeArea" :disabled="validDistrict" placeholder="区/县" v-model="district">
        <Option v-for="(district,index3) in area" :value="index3" :key="index3">{{district}}</Option>
      </Select>
    </div>
</template>
...
<script>
    export default {
      ...
      methods:{
          changeProvince(t){
            this.cities=this.addresses[t].city
            this.validCity=false
            this.validDistrict=true
            this.validDetail=true
            this.result.province=this.addresses[t].name
          },
        changeCity(t){
            this.area=this.cities[t].area
          this.validCity=false
          this.validDistrict=false
          this.validDetail=true
          this.result.city=this.cities[t].name
        },
        changeArea(t){
            this.result.area=this.area[t]
          this.validCity=false
          this.validDistrict=false
          this.validDetail=false
        },
        initialAddress(province,city,area){
            for(var i=0;i<this.addresses.length;i++){
              if(this.addresses[i].name===province){
                this.province=i
                this.cities=this.addresses[i].city
                break
              }
            }
